...
if not st.session_state.assistants:
    ...
    <html>
      ...
      <body>
        ...
        <script>
          ...
          const assistantId = '__ASSISTANT__';
          ...
          (async function(){
            try {
              ...
              var Vapi = window.Vapi && window.Vapi.default ? window.Vapi.default : window.Vapi;
              var vapi = new Vapi(apiKey);
              window._vapi=vapi;
              document.getElementById('start').onclick=function(){ vapi.start(assistantId); };
              document.getElementById('end').onclick=function(){ vapi.stop(); };
              document.getElementById('mute').onclick=function(){ vapi.mute(); };
              document.getElementById('unmute').onclick=function(){ vapi.unmute(); };
              vapi.on('call-start', function(){ log('call-start'); });
              vapi.on('call-end', function(){ log('call-end'); });
              vapi.on('message', function(m){ if(m && m.type==='transcript'){ log((m.role||'user')+': '+(m.transcript||'')); }});
    ...
    """)
    embedded_html=embedded_tpl.replace("__API_KEY__", vapi_public_key).replace("__ASSISTANT__", a_id)
    st.components.v1.html(embedded_html, height=260)

    # Full-screen Web SDK page in a new tab as fallback
    page_tpl=textwrap.dedent("""
    <!DOCTYPE html>
    <html>
      <head>
        <meta charset='utf-8' />
    ...
    """)
    data_url="data:text/html;base64,"+base64.b64encode(page_tpl.replace("__API_KEY__", vapi_public_key).replace("__ASSISTANT__", a_id).encode()).decode()
